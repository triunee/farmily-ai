...
# ── 텍스트 공통 정제 (- 기호 제거 + 공백 정리) ──────────────
def clean_text(text):
    lines = [
        line.strip().lstrip("-").strip()
        for line in text.splitlines()
        if line.strip() and line.strip() != "-" and len(line.strip()) > 1
    ]
    return " ".join(lines)

# ── 영양 필드: 표 제거 후 설명 텍스트만 추출 ─────────────────
...
def crawl_detail(detail_url, index, total):
    ...
    # dl/dt/dd 전체 파싱
    # 숫자/날짜 필드(월별, 월별농식품, 생산시기)는 clean_text 제외
    SKIP_CLEAN_KEYS = {"월별", "월별농식품", "생산시기"}
    ...
    for dl in view.select("dl"):
        ...
        if key == "영양":
            ...
            val = clean_text(dd.get_text(separator="\n"))
        data[key] = val
